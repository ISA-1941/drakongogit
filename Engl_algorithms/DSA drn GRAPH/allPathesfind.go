...
func  (graph  Graph) findPath(start int, last int, path string, visit[] bool)  {
    ...
    find++
    fmt.Println("   ######### Вход в findPath --> find, start,last -->  ", find, "  --> ", start, ",", last) 
    ...
    for {
        k++
        fmt.Println("   Cycle: k, start, edgePrev = ", k, start, ":", graph.node[start],  ": edge= ",edge)
        // item 132
        if edge != nil {
            
        } else {
            fmt.Println("-------------edge == nil ---------------")
            break
        }
        // item 134

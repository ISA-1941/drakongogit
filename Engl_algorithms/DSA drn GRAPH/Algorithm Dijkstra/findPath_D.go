...
var length int = math.MaxInt

func  (graph  Graph) allPath(start int, last int)  {
    // item 143
   fmt.Println("allPath-->", myMap)
    if graph.size <= 0 {
        // item 146
        return
    } else {
...
func  (graph  Graph) findPath(start int, last int, path string, visit[] bool)  {
    ...
    if start == last {
        ...
        fmt.Println("\n start == last: ")
        fmt.Print("(", path, ")")
		fmt.Print(" Длина строки = ",len(path))
		fmt.Println("")

    // Loop over the string with len.
    	for i := 0; i < len(path)-1; i=i+2 {
...
func  (graph *Graph) connect(start, last int)  {
    ...
    if graph.node[start].next == nil {
        ...
        // item 67
        graph.node[start].last.next = edge
        fmt.Println("graph.node[start].last.next -->", graph.node[start].last.next)
    }
    // item 68
    graph.node[start].last = edge
}
